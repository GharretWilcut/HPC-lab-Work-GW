...
int main(int argc, char **argv)
{
    ...
    // Collect DPU handles once before the BFS loop so threads can use them
    dpu_set_t dpuHandles[NR_DPUS];
    {
        dpu_set_t handle;
        int idx = 0;
        DPU_FOREACH(dpuSet, handle)
            dpuHandles[idx++] = handle;
    }

    // Allocate result buffers once, reuse every iteration
    ...
    do {
        ...
        //pull — parallel download into results[], then sequential merge
            ...
            for (int idx = 0; idx < NR_DPUS; idx++) {
                threads.emplace_back([&, idx]() {
                    ...
                    DPU_ASSERT(dpu_copy_from(dpu, DPU_MRAM_HEAP_POINTER_NAME,
                        m.p.changed_m, &r.changed64, sizeof(uint64_t)));
        ...
        // Phase 2: sequential merge into globals — no race conditions
